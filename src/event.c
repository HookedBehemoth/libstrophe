    ...
    while (connitem) {
        ...
        case XMPP_STATE_PROXY:
            if (FD_ISSET(conn->sock, &wfds)) {
                ...
                /* Request forwarding */
                prog = snprintf(buf, STROPE_MESSAGE_BUFFER_SIZE,
                ...
                if (conn->proxy_user) {
                    ...
                    auth_len =
                        snprintf(auth_buf, 0x48, "%s:%s", conn->proxy_user,
                                 conn->proxy_pass ? conn->proxy_pass : "");
                    ...
                    prog +=
                        snprintf(buf + prog, STROPE_MESSAGE_BUFFER_SIZE - prog,
                                 "Proxy-Authorization: Basic %s\r\n", enc);

                    xmpp_free(ctx, enc);
                }

                prog += snprintf(buf + prog, STROPE_MESSAGE_BUFFER_SIZE - prog,
                                 "\r\n");
